...
class Manager:
    ...
    def transition(self, config):
        ...
        def transTask(task):
            ...
            for obj in self.oldObjList:
                inst = obj[0]
                name = obj[1]
                if (name not in self.oldNamed) or self.oldNamed[name] != True:
    ...
    def addObj(self, element):
        ...
        if (
            name in self.oldNamed
            and isinstance(self.oldNamed[name], getattr(self.plugin[plugin], plugin))
            and getattr(self.oldNamed[name], "reload", None) != None
        ):
