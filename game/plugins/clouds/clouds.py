...
class CloudObj:
    ...
    def generate_sprites(self):
        ...
        for cloudobj in self.model.getChild(0).getChildren():
            ...
            tmpsoftness = random.gauss(self.softness, 0.1)
            num = 0
            if tmpsoftness <= 1 * 0.0625:
                pass
            elif tmpsoftness <= 2 * 0.0625:
                num = 1
            elif tmpsoftness <= 3 * 0.0625:
                num = 2
            elif tmpsoftness <= 4 * 0.0625:
                num = 3
            elif tmpsoftness <= 5 * 0.0625:
                num = 4
            elif tmpsoftness <= 6 * 0.0625:
                num = 5
            elif tmpsoftness <= 7 * 0.0625:
                num = 6
            elif tmpsoftness <= 8 * 0.0625:
                num = 7
            elif tmpsoftness <= 9 * 0.0625:
                num = 8
            elif tmpsoftness <= 10 * 0.0625:
                num = 9
            elif tmpsoftness <= 11 * 0.0625:
                num = 10
            elif tmpsoftness <= 12 * 0.0625:
                num = 11
            elif tmpsoftness <= 13 * 0.0625:
                num = 12
            elif tmpsoftness <= 14 * 0.0625:
                num = 13
            elif tmpsoftness <= 15 * 0.0625:
                num = 14
            else:
                num = 15
            ...
            if self.longest_dist < distance:
                self.longest_dist = distance
...
class Clouds:
    ...
    def __init__(self, manager, xml):
        ...
        if xmlcloudbox != None:
            self.cloudbox = (
                Point3(int(xmlcloudbox.get("x1")), int(xmlcloudbox.get("y1")), int(xmlcloudbox.get("z1"))),
                Point3(int(xmlcloudbox.get("x2")), int(xmlcloudbox.get("y2")), int(xmlcloudbox.get("z2"))),
            )
        if xmlcloudpos != None:
            self.cloudpos = Point3(int(xmlcloudpos.get("x")), int(xmlcloudpos.get("y")), int(xmlcloudpos.get("z")))
        ...
        for cloud in range(len(xmlcloudlist)):
            ...
            if self.cloudbox:
                ...
                pos = Point3(
                    random.randint(self.cloudbox[0].getX(), self.cloudbox[1].getX()),
