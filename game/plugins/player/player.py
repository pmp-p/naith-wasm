...
class Player:
    ...
    def playerPrePhysics(self):
        ...
        if targVel.lengthSquared() < 1e-2 and vel.lengthSquared() < 1e-1:
            ...
            targVel[
                2
            ] = 0.0  # Otherwise a vertical drop onto a slope can causes the player to do mini jumps to try and recover (!).
        else:
    ...
    def playerPostPhysics(self):
        ...
        pp = (
            self.body.getPosition() + self.body.getLinearVel() * self.ode.getRemTime()
        )  # Interpolation from physics step for smoother movement - due to physics being on a constant frame rate.
