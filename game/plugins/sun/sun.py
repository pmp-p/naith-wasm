    ...
    def __init__(self, manager, xml):
        ...
        if godrays != None:
            self.vlbuffer = base.win.makeTextureBuffer(
                "volumetric-lighting", int(base.win.getXSize() / 2), int(base.win.getYSize() / 2)
            )
            ...
            self.finalQuad.setAttrib(
                ColorBlendAttrib.make(ColorBlendAttrib.MAdd, ColorBlendAttrib.OIncomingColor, ColorBlendAttrib.OFbufferColor)
            )

            self.finalQuad.setShader(
                Shader.load(posixpath.join(manager.get("paths").getConfig().find("shaders").get("path"), "filter-vlight.cg"))
            )
