...
class SetAABB:
    ...
    def __init__(self, aabbs):
        ...
        # Get half the volume...
        totVolume = sum([x.volume for x in aabbs])
        ...
        for dim in range(3):
            ...
            options = (
                byDim[centre].bounds[dim][0] - aabbLambda,
                byDim[centre].centre[dim],
...
class Portal:
    ...
    def fromFace(self, aabb, dim, side):
        ...
        for index in square3d:
            coord = [aabb.bounds[d][index[d]] for d in range(3)]
            self.verts.append(coord)

        # If needed reorder them so its anticlockwise ordering from the view of the centre of the aabb...
        offsetC = [list(map(lambda a, b: a - b, x, aabb.centre)) for x in self.verts]
        ind = sum(
            [
                offsetC[1][i]
                * (offsetC[0][(i + 1) % 3] * offsetC[2][(i + 2) % 3] - offsetC[0][(i + 2) % 3] * offsetC[2][(i + 1) % 3])
                for i in range(3)
            ]
        )
    ...
    def setupPortal(self, portal, portalNode, flip):
        ...
        c = [sum([x[i] for x in self.verts]) / 4.0 for i in range(3)]
        ...
        for event in events:
            ...
            for key, aabb in state.items():
                ...
                if sum(withinCount) == 4:
                    if done:
                        raise Exception(
                            "Double interception - each culling aabb face can only intrecept one other cube as a fully contained face"
                        )
                    done = True
                ...
                elif len([x for x in withinCount if x > 0]) == 2:
